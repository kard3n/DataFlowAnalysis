 ...
 * {@link PrivacyDFDConfidentialityAnalysis}. The data contained in this class
 * is validated, when calling {@link PrivacyDFDDataFlowAnalysisBuilder#build()}
 * before an analysis object is returned
 */
public class PrivacyDFDDataFlowAnalysisBuilder extends DataFlowAnalysisBuilder {
	...
	protected String privacyModelPath;
	...
	/**
	 * Sets the privacy model used by the analysis
	 * 
	 * @return Builder of the analysis
	 */
	public PrivacyDFDDataFlowAnalysisBuilder usePrivacyModel(String privacyModelPath) {
		this.privacyModelPath = privacyModelPath;
		return this;
	}
	 ...
	 * 
	 * @param transposeFlowGraphFinderClass Custom TransposeFlowGraphFinder of the
	 *                                      analysis
	 */
	public PrivacyDFDDataFlowAnalysisBuilder useTransposeFlowGraphFinder(
	...
	/**
	 * Determines the effective resource provider that should be used by the
	 * analysis
	 */
	private PrivacyDFDResourceProvider getEffectiveResourceProvider() {
		if (this.customResourceProvider.isEmpty()) {
			URI dataDictionaryUri = this.modelProjectName.isEmpty()
					? URI.createFileURI(Paths.get(this.dataDictionaryPath).toAbsolutePath().toString())
			...
			URI dataFlowDiagramUri = this.modelProjectName.isEmpty()
					? URI.createFileURI(Paths.get(this.dataFlowDiagramPath).toAbsolutePath().toString())
					: ResourceUtils.createRelativePluginURI(this.dataFlowDiagramPath, this.modelProjectName);
			URI privacyModelUri = this.modelProjectName.isEmpty()
					? URI.createFileURI(Paths.get(this.privacyModelPath).toAbsolutePath().toString())
					: ResourceUtils.createRelativePluginURI(this.privacyModelPath, this.modelProjectName);

			return new PrivacyDFDURIResourceProvider(dataFlowDiagramUri, dataDictionaryUri, privacyModelUri);
		}
